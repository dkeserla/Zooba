...
#login manager initialization for getting different users
@login_manager.user_loader
def load_user(user_id):
    return User.query.get(int(user_id))

#user object that inherits from two database modules to be able to intialize Users through SQL Alchemy
#has all the various information about a given user for the site to function
class User(db.Model, UserMixin):
    ...
    #makes sure friends add each other when one accepts the others request
...
#intializes the course object for the SQL Alchemy database to store the necessary courses
#within these multiple courses there is various data about courses for student
